...
@section('content')
  ...
  <main class="h-full pb-16 overflow-y-auto">
    <div class="container px-6 mx-auto grid">
      ...
      @endif
      <!-- General elements -->
      <form method="POST" class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        @csrf
        @if ($errors->any())
        ...
        <label class="block text-sm">
          ...
          <input value="{{ old('name') ?? @$belonging->name }}" type="text" name="name" required
        ...
        <label class="block text-sm mt-2">
          ...
          <input value="{{ old('email') ?? @$belonging->email }}" type="email" name="email" required
            class="block w-full mt-1 text-sm border dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
        ...
        <label class="block text-sm mt-2">
          ...
          <input value="{{ old('phone') ?? @$belonging->phone }}" type="tel" name="phone" required
            class="block w-full mt-1 text-sm border dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
        ...
        <div class="mt-4 text-sm">
          ...
          <div class="mt-4 text-sm">
            ...
            <div class="mt-2">
              @foreach ($types as $type)
                <label class="inline-flex items-center text-gray-600 dark:text-gray-400">
                  <input @if (@$belonging->belonging_type_id == $type->id) checked @endif type="radio"
                    class="text-purple-600 border-2 form-radio focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray"
                    name="type" value="{{ $type->id }}" @if (old('type', @$belonging->belonging_type_id) == $type->id) checked @endif />
          ...
          <div class="mt-4 text-sm">
            ...
            <div class="mt-2">
              @foreach ($sizes as $size)
                <label class="inline-flex items-center text-gray-600 dark:text-gray-400">
                  <input @if (@$belonging->belonging_size_id == $size->id) checked @endif type="radio"
                    class="text-purple-600 border-2 form-radio focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray"
                    name="size" value="{{ $size->id }}" @if (old('size', @$belonging->belonging_size_id) == $size->id) checked @endif />
          ...
          <label class="block text-sm mt-2">
            ...
            <input value="{{ old('color_name') ?? @$belonging->color_name }}" type="hidden" required name="color_name"
              id="color_name">
            <input value="{{ old('color') ?? @$belonging->color }}" id="color" type="color" name="color"
              class="block w-full mt-1 text-sm border dark:focus:shadow-outline-gray " placeholder="#ff0000" />
            <p id="color_name_p" style="color: {{ old('color') ?? @$belonging->color }}">
          ...
          <label class="block mt-4 text-sm">
            ...
            <textarea name="notes"
              ...
              rows="3" placeholder="(Optional)">{{ old('notes') ?? @$belonging->notes }}</textarea>
          </label>
          @if (!isset($belonging))
            <div class="mt-4 text-sm">
              <span class="text-gray-700 dark:text-gray-400">
              ...
              <div class="mt-2">
                @foreach ($slots as $slot)
                  <label
                    class="
                    @if ($slot->max - $slot_counts[$slot->name] <= 0) opacity-50 @endif
                    inline-flex relative rounded-full bg-gray-300 items-center text-gray-600 dark:text-gray-400">
                    <input @if (old('slot_id') == $slot->id) checked @endif
                      @if ($slot->max - $slot_counts[$slot->name] <= 0) disabled @endif type="radio"
                    ...
                    <span class="pl-8 ml-2 mr-8">{{ $slot->name }}
                      ({{ $slot->max - $slot_counts[$slot->name] }})
                    </span>
                  </label>
                @endforeach
              </div>
        ...
        <button type="submit"
          class="table items-center mt-4 justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
          {{ isset($belonging) ? 'Edit Belonging' : 'Add To Vault' }}
